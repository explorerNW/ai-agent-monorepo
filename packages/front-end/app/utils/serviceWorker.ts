...
export async function checkServiceWorkerStatus(): Promise<ServiceWorkerStatus> {
  ...
  if (status.isRegistered) {
    ...
    if (!status.isActive && registration?.installing) {
      console.log('[SW] ⏳ Service Worker is installing... Please wait or refresh the page.');
    }
  } else {
    ...
    if (hasVisitedBefore) {
      console.warn('[SW] ⚠️ No registration found on repeat visit - SW may have failed to install');
    } else {
      console.log('[SW] ℹ️ No registration found - this is normal on first visit');
  ...
  if (!status.isActive) {
    ...
    if (!isHTTPS && !isLocalhost) {
      ...
      if (!status.isRegistered) {
        ...
        console.warn('[SW] If Service Worker fails to register, you may need to:');
      ...
      console.log('[SW] ℹ️ Service Worker may still be activating (normal on first load)');
 ...
 */
export async function removeFromCache(url: string, cacheName?: string): Promise<void> {

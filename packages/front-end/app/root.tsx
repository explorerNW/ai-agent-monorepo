  ...
  useEffect(() => {
    ...
    const registerServiceWorker = async (retryCount = 0) => {
      ...
      try {
        ...
        console.error('[SW] ❌ Registration failed:', (error as Error).message);

        // Check if it's a security/certificate issue
        if (
          (error as Error).name === 'SecurityError' ||
          (error as Error).message.includes('secure origin')
        ) {
        ...
        if (retryCount < maxRetries && !(error as Error).message.includes('security')) {
          console.log(`[SW] 🔄 Retrying registration (${retryCount + 1}/${maxRetries})...`);
          ...
          console.error('[SW]   - Try hard refresh (Ctrl+Shift+R / Cmd+Shift+R)');
        }
      }
    };
